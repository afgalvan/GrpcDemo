...
namespace ServiceLib
{
    ...
    {
        private readonly
            ConcurrentDictionary<string, IServerStreamWriter<ChatMessage>>
            _joinedUsers;
        ...
        public ChatHub(ILogger<ChatHub> logger)
        {
            _joinedUsers =
                new ConcurrentDictionary<string,
                    IServerStreamWriter<ChatMessage>>();
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public Task HandleIncomingMessage(ChatMessage message,
                ...
                if (_joinedUsers.TryGetValue(user, out storedResponseStream))
                {
                    if (storedResponseStream != responseStream)
                    {
                        throw new InvalidOperationException(
                            $"A user {user} is already in the chat!");
                    }
                }
                else
                {
        ...
        private async Task DistributeMessage(ChatMessage message)
        {
            ...
            foreach (var reveiver in _joinedUsers.Where(u =>
                u.Key != message.User))
            {
